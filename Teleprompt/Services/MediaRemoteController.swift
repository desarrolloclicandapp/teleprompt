...
private final class BluetoothGamepadFallbackController: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    private func parseAndEmit(data: Data) {
        ...
        if (pressed & 0x01) != 0 { onAction?(.playPause) }
        if (pressed & 0x02) != 0 { onAction?(.reset) }
        if (pressed & 0x04) != 0 { onAction?(.toggleRecordingPause) }
        if (pressed & 0x08) != 0 { onAction?(.next) }
        if (pressed & 0x10) != 0 { onAction?(.decreaseSpeed) }
        if (pressed & 0x20) != 0 { onAction?(.increaseSpeed) }
